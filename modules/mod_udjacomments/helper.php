...
class modUdjaCommentsHelper
{
	...
	public function __construct()
	{
		...
		if ($this->getParam('use_css') == 1)
		{
			$document->addStyleSheet(JURI::Base(true).'media/mod_udjacomments/css/mod_udjacomments.css','text/css','screen');
		}
		
		//do we want to enable replies?
		if ($this->getParam('enable_reply',1) == 1)
		{
			$document->addScript('https://ajax.googleapis.com/ajax/libs/jquery/1.6.1/jquery.min.js');
			$document->addScriptDeclaration('jQuery.TEXT = { REPLY_TO_COMMENT : \''.JText::_('MOD_UDJACOMMENTS_REPLY_TO_COMMENT').'\' }', 'text/javascript');
	...
	public function getViewable()
	{
		...
		{			
			...
			if ($catid == 0)
			{
				...
				$sql = "SELECT catid FROM #__content WHERE id = ".$tid;
				$database->setQuery( $sql );
				$row=$database->loadResult();
				if ($this->params->get('enable_catlist',0) == 1 && !in_array($row, $this->params->get('cat_list'))) { return false; }
			}
			else
			{
				if ($this->params->get('enable_catlist',0) == 1 && !in_array(JRequest::getInt('catid'), $this->params->get('cat_list'))) { return false; }
			}
		}
		return true;		
	}
	...
	public function getParam($field, $default = 0)
	{
		return $this->params->get($field, $default);
	}
	...
	public function saveComment()
	{
		...
		if ((!$email = JRequest::getString('txtUdjaEmail')) || (!$this->getValidEmail($email))) { $errList[] = JText::_('MOD_UDJACOMMENTS_VALIDATION_TXTEMAIL'); }
		...
		//now to save the comment! - Or wait, is it spam?
		//run spam checker
		...
		//use component/id combo if available
		if ($com_name = JRequest::getString('option') && $com_id = JRequest::getInt('id')) { $comment_url = JRequest::getString('option') .':'. JRequest::getInt('id'); }
	...
	public function isSpam()
	{
		...
		if (JRequest::getString('txtUdjaWebsite') && in_array(JRequest::getString('txtUdjaWebsite'), $urlList)) { return 1; }
		if (JRequest::getString('txtUdjaEmail') && in_array(JRequest::getString('txtUdjaEmail'), $emailList)) { return 1; }
	...
	public function getCommentList($url, $parentId=0)
	{
		...
		$limit = ($parentId != 0) ? '' : ' LIMIT '.$pageStart.','.($pageStart+$this->params->get('per_page',10));
		...
		$sql = 'SELECT
				...
				WHERE
					(
						...
						OR
						`comment_url`="'.JRequest::getString('option').':'.JRequest::getInt('id').'"
					)					
				AND
					`parent_id`='.$parentId.'
				AND
		...
		if ($dbo->Query())
		{
			...
			foreach ($dbo->loadObjectList() as $comment)
			{
				...
				//replace [quote] with <cite> tags
				$comment->content = preg_replace('/\[quote\](.*?)\[\/quote\]/is','<cite>$1$2</cite>',$comment->content);
				
				//replace [code] with <code> tags
				$comment->content = preg_replace('/\[code(.*)\](.*?)\[\/code\]/is','<code>$1$2</code>',$comment->content);
	...
	{
		if ($this->params->get('enable_notifications',1) && $this->isPublish() == 1 && $this->isSpam() != 1)
		{
			$dbo = JFactory::getDbo();
			$sql = 'SELECT
						`email`,
			...
			if ($dbo->Query())
			{
				...
				foreach ($dbo->loadObjectList() as $notify)
				{
					if (!in_array($notify->email, $sentList) && $notify->email != JRequest::getString('txtUdjaEmail'))
					{
						...
						$unsubscribeUrl = JURI::base() . $commentUrl . ((stristr($commentUrl,'?')) ? '&' : '?');
						if ($option = JRequest::getString('option') && $option_id = JRequest::getString('id'))
						{
							$unsubscribeUrl .= 'unsubscribe=' . base64_encode(JRequest::getString('option').':'.JRequest::getInt('id'));
						}
						else
						{
							$unsubscribeUrl .= 'unsubscribe=' . base64_encode($commentUrl);
						}
						$unsubscribeUrl .= '&email=' . base64_encode($notify->email);
	...
	public function setUnsubscribe($url, $email)
	{
		$dbo = JFactory::getDbo();
		...
		$sql = 'UPDATE
				...
				WHERE
					`comment_url` = "'.$dbo->getEscaped(base64_decode($url)).'"
				AND
					`email` = "'.$dbo->getEscaped(base64_decode($email)).'"';
		$dbo->setQuery($sql);
		if ($dbo->Query())
		{
			return true;
