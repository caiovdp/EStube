...
foreach ($commentList as $comment)
{
	...
	echo '<li id="comment'.$comment->id.'" class="'.$liClass.'">';
		...
		echo '<div class="commentDetails">';
			...
			//incase an import provided no name:
			if (isset($comment->full_name) && $comment->full_name == '') { $comment->full_name = 'Anonymous'; }
			
			//commenter name (link if applicable)
			if ($comment->url) {
				echo '<p class="commentName"><a rel="nofollow" href="'.htmlspecialchars((stristr($comment->url,'http')) ? $comment->url : 'http://'.$comment->url, ENT_QUOTES, 'UTF-8').'" target="_blank" title="'.$comment->full_name.'">'.$comment->full_name.'</a></p>';
			} else {				
				echo '<p class="commentName">'.$comment->full_name.'</p>';
			}
